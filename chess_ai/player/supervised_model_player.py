...
def get_agent(
    ...
    if not os.path.exists(model_path):
        agent_trainer = SupervisedModelTrainer(model_folder=model_folder)
        agent = agent_trainer.train(
            batch_size=batch_size, num_epochs=num_epochs, embedding_dim=embedding_dim, num_head=num_head
        )
...
def play_a_supervised_model_game(
    ...
    agent = get_agent(model_folder=model_folder, num_epochs=num_epochs, embedding_dim=embedding_dim, num_head=num_head)
